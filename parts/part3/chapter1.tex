...
transfers happen concurrently with compute (Figure~\ref{fig:prefetch}). In both
...
\begin{figure}
\begin{center}
\includegraphics[width=\columnwidth]{figures/part3-chp1/prefetch_diagram.pdf}
\end{center}
...
to \sfs in a worst case scenario. Furthermore, as all data reads are considered
...
been prefetched (Algorithm~\ref{alg:prefetch}). Each block will be written to an
...
($verify\_used()$) and queries the filesystem to determine which blocks have
...
break; } } \caption{Prefetching}\label{alg:prefetch}
\end{algorithm2e}
...
the filesystem at time of removal. We then update the list to ensure that we do
...
be the sum of three components (Equation~\ref{eq:s3fs}): 1) latency-associated
...
Rolling Prefetch contrasts Equation~\ref{eq:s3fs} in that the compute and data
transfer times mask one another (Equation~\ref{eq:prefetch}). However, Rolling
...
% the original as I found it:}
 Each \texttt{.trk} file is comprised of a 1,000-byte header and a body of
...
\subsubsection{Neuroimaging use cases}\label{exp:4} Our previous experiments
...
\subsection{Varying the block size}%% some parts may be better suited for
discussion
...
\begin{figure}
\begin{center}
\includegraphics[width=\columnwidth]{figures/part3-chp1/speedup.pdf}
