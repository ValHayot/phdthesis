\begin{abstract}
  ...
  practices coupled with improved data collection infrastructure enabling higher
  resolution imaging. Whereas the pipelines have shifted to become increasingly
  data-intensive, the software has minimally adapted to address this shift.
  Rather, scientific frameworks have primarily focused on ease-of-use,
  reproducibility, portability and parallelism. While the goals of Big Data and
  scientific frameworks differ, they can be combined to make scientific
  frameworks more suitable to the processing of the increasingly prominent
  scientific Big Data.
...
most data-intensive case. While we found Big Data frameworks (i.e. Apache Spark)
...
solutions, such as Intel Optane DCPMM produce speedups similar to processing
entirely in memory with Spark and could be used as an alternative, it remains
...
results demonstrate the Rolling Prefetch can speed up experiments by a factor of
1.8$\times$ and has a theoretical bound of 2$\times$.

Sea targets the standard neuroimaging workflows executed on HPC. It is able to
bring prefetching, data-locality and in-memory computing to POSIX-based
command-line tools through the interception of libc calls. Using this approach
...
show that Sea can speed up execution by an average of 7$\times$ with large datasets
