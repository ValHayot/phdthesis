    ...
    The rise of Big Data has transformed the way in which data is processed. This chapter aims to discuss the main differences between current neuroimaging/scientific and Big Data frameworks. In Section~\ref{status}, we will briefly introduce the two types of datasets that are
    giving rise to the neuroimaging big data, the available data-sharing and
    processing platforms and the differences between
    commodity and HPC infrastructure.
    As we wish to compare scientific workflow engines with Big Data frameworks for the
    processing of neuroimaging big data, we will break down our analyses by the four principal features necessary
    for scientific data processing: 1) workflow composition (Section~\ref{workcomp}), 2) performance (Section~\ref{performance}), 3) portability (Section~\ref{portability}) and 4) reproducibility (Section~\ref{reproducibility}).
    \section{Current state of neuroimaging applications}\label{status}
    ...
    \section{Workflow composition}\label{workcomp}
        ...
        \subsection{Workflow language}\label{lang} The choice of workflow
            ...
            features result in the workflow engine Make being desirable for scientific
    ...
    \section{Performance}\label{performance} Neuroimaging applications are
        ...
        use of commodity clusters, where data transfers over networks can be cumbersome.
        ...
        \subsection{Big Data Scheduling}\label{sched}
            ...
            YARN and Mesos, Spark provides its own default (standalone) scheduler.
    Similarly to scientific
            ...
            has two execution modes: client and cluster. In client execution mode,
            the application driver is intended to be executed on the client
            node. In contrast, the driver is intended to be running
            on a compute node, in cluster mode. Cluster mode can ensure fault-tolerance of not
...
\newcolumntype{s}{>{\columncolor{red!20}} c}
\begin{tabular}{|c|b|s|}
    \cline{2-3}
    \multicolumn{1}{c|}{} & \textbf{Big Data Frameworks} & \textbf{Scientific workflow engines} \\
\hline
\textbf{Data representation} & Distributed data structures & Files and primitive types \\
\hline
\textbf{Fault-tolerance} & Lineage & Checkpoint/Restart \\
\hline
\textbf{Tool interface} & None & Programming/Domain-specific languages \\
\hline 
\textbf{Provenance} & Lineage & W3C-Prov/NIDM-results/Custom \\
\hline 
\end{tabular}
} \caption{Differences between Big Data frameworks and scientific workflow engines according to neuroimaging workflow criteria}
\label{tb:summary}
\end{table*}
    
        Although numerous neuroimaging workflow engines exist, none of them are well
        ...
        ease-of-use, performance, portability and reproducibility. Table~\ref{tb:summary} demonstrates the differences in how Big Data frameworks and scientific workflow engines meet this criteria. In order 
